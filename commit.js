...
console.log("Release Agent Commit Routine");
updateVersion();
updateData();
...
function updateVersion() {
	...
	if (parts.length != common.getVersionFormatStructure().length) {
		exit('Unexpected version format found in version file. Edit conf file or manually change version file to match 4 parts.');
	}
	var result = [];
	...
	result = result.join('.');
	common.writeFile(common.getConf().versionFilePath, result, function() {
...
function handleAttribute(letter, as) {
	...
	while (commitMsg.toUpperCase().indexOf(search, currentPos) != -1) {
		...
		common.writeData(as, msg, function() {
